...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  return (x1 * y1 + x2 * y2) / ((getDistanceBetweenPoints(0, 0, x1, y1) * getDistanceBetweenPoints(0, 0, x2, y2));
}
...
function toNumber(value, def) {
  return parseFloat(value) === NaN ? def : parseFloat(value);
}
...
function getSumToN(n) {
  if (n % 2 === 0) {
    return (n.slice(0) + n.slice(-1)) * (n / 2);
  } else {
    return (n.slice(0) + n.slice(-1)) * (n / 2) + n.slice(n / 2);
  }
}
...
function toExponential(number, fractionDigits) {
  return `${number.toString().slice(0)}.${number.toString().slice(1, fractionDigits + 1)}e+${number.length - 1}`;
}
